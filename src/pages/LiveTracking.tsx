...
const LiveTracking = () => {
  ...
  return (
    <div className="container mx-auto max-w-2xl py-6 md:py-12 pb-24 px-4 relative overflow-hidden">
       {/* Background Orbs */}
       <div className="absolute top-[20%] right-[-10%] h-[400px] w-[400px] rounded-full bg-primary/5 blur-[120px] pointer-events-none" />
      ...
      <div className="mb-10 flex items-center justify-between relative z-10">
          <div>
            ...
            <p className="text-muted-foreground mt-2 font-bold text-[10px] uppercase tracking-widest pl-1">Quantum State: {job.status}</p>
          </div>
          <button onClick={() => navigate("/verify")} className="p-3 rounded-2xl bg-card/5 border border-border text-muted-foreground hover:text-primary-foreground transition-all">
             <ArrowLeft size={24} />
          </button>
      </div>

      <div className="rounded-[3rem] bg-background border border-border p-12 text-center shadow-2xl relative overflow-hidden backdrop-blur-xl mb-10 group">
        ...
        <h3 className="text-3xl font-black mb-4 tracking-tighter italic text-primary-foreground uppercase italic">
        ...
        <p className="text-[10px] font-bold text-muted-foreground max-w-[320px] mx-auto mb-12 leading-relaxed uppercase tracking-[0.2em] italic">
        ...
        <div className="flex items-center justify-between relative mb-16 px-4">
           <div className="absolute left-0 right-0 top-1/2 h-[2px] bg-card/5 -translate-y-1/2 z-0 mx-12 rounded-full" />
           {steps.map((s, i) => {
             ...
             return (
               <div key={s.key} className="relative z-10 flex flex-col items-center gap-4">
                 ...
                 <span className={`text-[8px] font-black uppercase tracking-[0.3em] ${isActive ? "text-primary" : "text-foreground"}`}>
                   {s.label}
                 </span>
               </div>
             );
        ...
        {/* Worker Info Card */}
        {job.workerName && (
          <div className="bg-card/5 rounded-[2.5rem] p-8 border border-border text-left flex items-center gap-6 animate-fade-in shadow-inner group-hover:border-primary/20 transition-all mb-10">
             ...
             <div className="flex-1 min-w-0">
                <div className="text-[9px] font-black text-muted-foreground uppercase tracking-[0.4em] mb-2 pl-1">Primary Match</div>
                <div className="text-xl font-black text-primary-foreground truncate italic uppercase tracking-tighter">{job.workerName}</div>
                {job.workerSkill && (
             ...
             <div className="flex gap-3">
                <a href={`tel:${job.workerPhone}`} className="h-14 w-14 rounded-2xl bg-primary/10 text-primary flex items-center justify-center hover:bg-primary hover:text-primary-foreground transition-all shadow-lg border border-primary/20">
                   <Phone size={22} />
                </a>
                <button className="h-14 w-14 rounded-2xl bg-card/5 text-primary-foreground flex items-center justify-center hover:bg-card/10 transition-all shadow-lg border border-border">
        ...
        <div className="space-y-6">
               ...
               {!showReviewForm ? (
                 ...
                 <div className="p-8 rounded-[2rem] bg-card/5 border border-emerald-500/20 animate-in slide-in-from-bottom-3 duration-300 text-left">
                   ...
                   <textarea
                     ...
                     className="w-full p-4 rounded-2xl bg-background/40 border border-border text-sm text-primary-foreground placeholder:text-foreground outline-none focus:border-primary/50 resize-none h-24 mb-6"
                   />

                   <div className="flex gap-4">
                     <button
                       onClick={() => setShowReviewForm(false)}
                       className="flex-1 h-14 rounded-2xl bg-card/5 border border-border text-muted-foreground font-black uppercase tracking-widest text-[10px] hover:text-primary-foreground transition-all"
                     >
                       Cancel
                     </button>
                     <button
           ...
           <div className="flex items-center justify-center gap-3 text-[9px] font-black text-foreground py-2 uppercase tracking-[0.3em] italic">
      ...
      <div className="rounded-[2.5rem] bg-background p-8 border border-border relative overflow-hidden">
         <div className="absolute -top-12 -right-12 h-24 w-24 bg-primary/5 rounded-full blur-2xl" />
         <h4 className="font-black text-[10px] text-muted-foreground mb-6 flex items-center gap-3 uppercase tracking-[0.3em] pl-1 relative z-10">
         ...
         <div className="space-y-6 relative z-10">
            <div className="flex items-start gap-4">
               ...
               <p className="text-[10px] text-muted-foreground font-bold uppercase tracking-tight leading-relaxed">Worker generates a job-specific QR. Scan it to verify completion and leave a review.</p>
            </div>
            <div className="flex items-start gap-4">
               <div className="h-1.5 w-1.5 rounded-full bg-primary mt-1.5 shrink-0" />
               <p className="text-[10px] text-muted-foreground font-bold uppercase tracking-tight leading-relaxed">Job is marked complete ONLY after your review, ensuring accountability for both parties.</p>
            </div>
         </div>
      </div>
    </div>
