    ...
    <div className="container mx-auto max-w-7xl py-6 md:py-10 pb-24 px-4 relative overflow-hidden">
      ...
      <div className="mb-12 relative z-10 text-center">
        ...
        <div className="inline-flex items-center gap-3 px-6 py-2 rounded-full bg-primary/10 border border-primary/20 mb-6">
          ...
          <span className="text-[10px] font-black text-primary uppercase tracking-[0.4em]">Live Data • Real-Time</span>
        </div>
        <h1 className="text-3xl sm:text-5xl md:text-6xl font-black italic tracking-tighter text-primary-foreground uppercase">
          Mukti <span className="text-primary">Impact</span>
        </h1>
        <p className="text-sm font-bold text-muted-foreground uppercase tracking-widest mt-4 max-w-lg mx-auto">
      ...
      <div className="relative z-10 mb-10">
        <div className="rounded-[3rem] bg-gradient-primary p-8 sm:p-10 md:p-14 text-center shadow-2xl shadow-primary-glow relative overflow-hidden">
          ...
          <div className="relative z-10">
            ...
            <div className="text-3xl sm:text-5xl md:text-7xl font-black text-primary-foreground italic tracking-tighter">
              ₹{creditUnlocked.toLocaleString('en-IN')}
            </div>
            <div className="text-[10px] font-black text-primary-foreground/50 uppercase tracking-[0.3em] mt-3">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4 md:gap-6 relative z-10 mb-10">
        {loading ? (
          <div className="col-span-full py-20 text-center">
            <div className="animate-pulse text-muted-foreground font-bold uppercase tracking-widest text-[10px]">Computing Impact...</div>
          </div>
        ) : impactCards.map((card, i) => (
          <div key={i} className="rounded-[2rem] bg-background border border-border p-4 sm:p-6 md:p-8 text-center group hover:border-border transition-all">
            ...
            <div className="text-2xl sm:text-3xl md:text-4xl font-black text-primary-foreground italic tracking-tighter">{card.value}</div>
            <div className="text-[8px] font-black text-muted-foreground uppercase tracking-[0.2em] mt-2">{card.label}</div>
          </div>
        ))}
      </div>
